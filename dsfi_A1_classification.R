...
m_svm <- svm(train_y~train_X, kernel='radial', type='C-classification')
  
  #svm(train_y ~ train_X, kernel = "polynomial", degree=1)
predictions <- predict(m_svm, newdata = train_X)
mean(predictions==train_y)

confusion_matrix <- table(Actual = train_y, Predicted = predictions)
...
m_cnn <- keras_model_sequential() %>%
  layer_conv_2d(filters = 32, kernel_size = c(3,3), activation = "relu", input_shape = c(dim(train_X)[1], dim(train_X)[2], 1)) %>%
...
# Train the model (assuming you have train_data and train_labels)
history <- model %>% fit(
...
test_loss_and_metrics <- model %>% evaluate(test_data, test_labels)
